...
# 4) Vérification optionnelle des variables Drive (non bloquant)
if not os.getenv("FOLDER_ID"):
    print("⚠️ FOLDER_ID non défini (Drive backup désactivé)")
if not os.getenv("GOOGLE_APPLICATION_CREDENTIALS"):
    print("⚠️ GOOGLE_APPLICATION_CREDENTIALS non défini (Drive backup désactivé)")
...
def log_request_info():
    app.logger.info(f"Requête entrante: {request.method} {request.url}")
...
if google_api_key:
    try:
        ...
        print("✅ Google Gemini client initialisé (gemini-pro-vision)")
    except Exception as e:
...
# --- Fonction IA: Génération avec Gemini ---
def generate_ai_coaching(prompt_content, max_tokens=500):
    """
    Fonction générique universelle pour générer du contenu avec Gemini

    Args:
        prompt_content: Le prompt à envoyer à Gemini
        max_tokens: Nombre maximum de tokens (non utilisé par Gemini, mais conservé pour compatibilité)
...
def generate_past_week_comment(past_week_analysis):
    ...
    """
    if not gemini_model or not past_week_analysis:
        return None
...
def format_coaching_comment_as_html(coaching_text):
    """
    Formatage simple et lisible du commentaire coaching.
    """
    # Convertir les sauts de ligne en <br> et garder le texte simple
    lines = coaching_text.strip().split('\n')
    formatted_lines = []
    ...
    # Retourner avec des <br> pour les sauts de ligne
    return '<div style="line-height: 1.6; color: #333; font-size: 0.95rem;">' + '<br><br>'.join(formatted_lines) + '</div>'
    ...
    if run_date_iso:
        try:
            ...
            if weekly_plan and 'runs' in weekly_plan:
                ...
                # Chercher le run planifié pour cette date
                for planned in weekly_plan['runs']:
                    if planned.get('day_date') == run_date_str:
                        planned_run = planned
                        print(f"🎯 Objectifs planifiés trouvés pour {run_date_str}: {planned.get('type_display')}")
                        break
                if not planned_run:
                    print(f"⚠️ Pas de run planifié pour la date exacte {run_date_str}")
            else:
...
"""
    ...
    if is_last_run_of_week and activities and run_date_iso:
        ...
        try:
            ...
            # Générer le résumé de la semaine avec comparaison programmé vs réalisé
            if week_runs:
                ...
                # Charger le programme de la semaine pour comparaison
                weekly_plan_current = read_weekly_plan(current_week)
                programmed_distance = 0
                programmed_runs_count = 0
                if weekly_plan_current:
                    programmed_distance = weekly_plan_current.get('summary', {}).get('total_distance', 0)
                    programmed_runs_count = len(weekly_plan_current.get('runs', []))
                    print(f"📊 Programme semaine {current_week} chargé: {programmed_distance} km, {programmed_runs_count} runs")

                week_summary = f"\n\n=== BILAN SEMAINE {current_week} - RÉALISÉ vs PROGRAMMÉ ===\n\n"
                week_summary += f"VOLUME:\n"
                week_summary += f"  Runs: {len(week_runs)}/{programmed_runs_count} programmés\n"
                week_summary += f"  Distance: {total_distance:.1f} km (programmé: {programmed_distance:.1f} km, écart: {total_distance - programmed_distance:+.1f} km)\n"
                week_summary += f"  Dénivelé: {total_dplus:.0f} m\n\n"
                week_summary += f"QUALITÉ MOYENNE:\n"
                week_summary += f"  k moyen semaine: {avg_k:.2f}\n"
                week_summary += f"  Dérive moyenne semaine: {avg_drift:.1f}%\n\n"

                week_summary += "DÉTAIL RUNS RÉALISÉS vs PROGRAMMÉS:\n"

                # Créer un mapping des runs programmés par jour
                programmed_by_day = {}
                if weekly_plan_current:
                    for prog_run in weekly_plan_current.get('runs', []):
                        day_date = prog_run.get('day_date', '')
                        programmed_by_day[day_date] = prog_run
                ...
                for i, run in enumerate(week_runs, 1):
                    ...
                    run_type_cat = run.get('session_category', run.get('type_sortie', 'N/D'))

                    week_summary += f"\n  Run {i} ({run_date}, {run_type_cat}):\n"
                    week_summary += f"    Réalisé: {run_dist:.1f}km @ {run_allure}/km, k={run_k:.2f}, dérive={run_drift:.1f}%\n"

                    # Comparer avec programmé si disponible
                    prog = programmed_by_day.get(run_date)
                    if prog:
                        prog_dist = prog.get('distance_km', 0)
                        prog_pace = prog.get('pace_target', '-:--')
                        prog_k = prog.get('k_target', 0)
                        prog_drift = prog.get('drift_target', 0)
                        week_summary += f"    Programmé: {prog_dist:.1f}km @ {prog_pace}/km, k<{prog_k:.2f}, dérive<{prog_drift:.1f}%\n"
                        week_summary += f"    Écarts: distance {run_dist - prog_dist:+.1f}km, k {run_k - prog_k:+.2f}, dérive {run_drift - prog_drift:+.1f}%\n"
                    else:
                        week_summary += f"    (Pas de run programmé ce jour)\n"
        except Exception as e:
            week_summary = f"\n[Erreur génération bilan semaine: {e}]\n"
            print(f"❌ Erreur bilan semaine: {e}")

    # Formatter les objectifs planifiés si disponibles
    planned_objectives_text = ""
    if planned_run:
        planned_objectives_text = f"""
=== OBJECTIFS PLANIFIÉS (Weekly Plan) ===
...
Drift cible: {planned_run.get('drift_target', 'N/D')}%
...
"""
    ...
    try:
        ...
        print(f"DEBUG: Using model {gemini_model.model_name}")
...
# ═══════════════════════════════════════════════════════════════════
# FONCTIONS DE COMPATIBILITÉ (à supprimer progressivement)
# ═══════════════════════════════════════════════════════════════════

def normalize_session_type(activity):
    """
    DEPRECATED - Utilisé uniquement pour compatibilité programme hebdo.
    Mappe les nouveaux types vers anciens types.
    """
    type_sortie = classify_run_type(activity)

    # Mapping vers anciens types programme
    mapping = {
        'long_run': 'sortie_longue',
        'endurance': 'tempo',
        'tempo_rapide': 'tempo',
        'tempo_recup': 'recuperation',
        'inconnue': 'tempo'
    }

    return mapping.get(type_sortie, 'tempo')


def detect_session_type(activity):
    """
    DEPRECATED - Appelle classify_run_type().
    Gardé pour compatibilité avec ancien code.
    """
    return classify_run_type(activity)
...
# ═══════════════════════════════════════════════════════════════════
# DÉTECTION FRACTIONNÉ - DÉSACTIVÉ (XGBoost trop complexe/pas fiable)
# ═══════════════════════════════════════════════════════════════════

def apply_fractionne_flags(activities):
    """
    DÉSACTIVÉ - XGBoost supprimé pour simplification.
    Garde l'interface pour compatibilité mais ne fait rien.
    """
    return activities, False

def apply_fractionne_flags_OLD(activities):
    """
...
def add_historical_context(activities):
    ...
    def get_session_category(act):
        ...
        if is_fractionne:
            ...
            # Reclassification automatique selon nouvelle typologie
            return detect_session_type(act)
        return None
...
def generate_segment_analysis(activity, feedback, profile, segments, patterns,
    ...
    Returns:
        str: Commentaire coaching IA (4 lignes max)
    """
    # 🆕 Charger le nouveau prompt de coaching
    prompt_template = load_prompt("coaching_run")
    if not prompt_template:
        return "⚠️ Template de prompt introuvable."

    # 🆕 Calculer les moyennes par type de run (10 derniers)
    run_type = classify_run_type(activity)

    # Calculer moyennes pour tous les types de runs
    type_averages = {}
    for run_t in ['tempo_recup', 'tempo_rapide', 'endurance', 'long_run']:
        avg = calculate_type_averages(activities, run_t, limit=10)
        type_averages[run_t] = avg
    ...
    zones_avg_data = {z: 0 for z in range(1, 6)} # Initialiser avec des valeurs par défaut
    if activities and type_run: # `activities` et `type_run` sont disponibles en paramètre
        same_type_zones = {1: [], 2: [], 3: [], 4: [], 5: []}
        # Itérer sur les activités pour trouver les données historiques
        for act_hist in activities[:50]:  # 50 dernières activités
            # Assurez-vous que l'activité historique a bien une analyse cardiaque
            if (act_hist.get('session_category') == type_run or act_hist.get('type_sortie') == type_run) and act_hist.get('cardiac_analysis'):
                act_zones = act_hist['cardiac_analysis'].get('hr_zones', {}).get('zone_percentages', {})
                if act_zones:
                    for z in range(1, 6):
        ...
        for z in range(1, 6):
            if same_type_zones[z]:
                zones_avg_data[z] = sum(same_type_zones[z]) / len(same_type_zones[z])
            else:
                zones_avg_data[z] = 0

    # Formatter les moyennes pour le prompt
    zones_avg_str = ", ".join([f"Z{z}: {zones_avg_data.get(z, 0):.1f}%" for z in range(1, 6)]) if zones_avg_data else "N/D"

    zone_pcts = {} # Initialiser comme dict vide
    ...
    week_summary = ""
    current_week = 0
    print(f"🔍 is_last_run_of_week = {is_last_run_of_week}, activities count = {len(activities) if activities else 0}, date = {date}")
    if is_last_run_of_week and activities and date:
        ...
        try:
            ...
            if week_runs:
                ...
                # Charger le programme de la semaine pour comparaison
                weekly_plan = read_weekly_plan()
                programmed_distance = 0
                programmed_runs_count = 0
                if weekly_plan:
                    programmed_distance = weekly_plan.get('summary', {}).get('total_distance', 0)
                    programmed_runs_count = len(weekly_plan.get('runs', []))
                    print(f"📊 Programme semaine {current_week} chargé: {programmed_distance} km, {programmed_runs_count} runs")
                ...
                programmed_by_day = {}
                if weekly_plan:
                    for prog_run in weekly_plan.get('runs', []):
                        day_date = prog_run.get('day_date', '')
                        programmed_by_day[day_date] = prog_run
    ...
    next_week = current_week + 1 if current_week > 0 else 1
    if next_week > 52:
        next_week = 1
...
def index():
    ...
    type_count = 0
    for idx, activity in enumerate(activities):
        # 1) Classifier TOUS les runs avec le nouveau système simplifié
        old_type = activity.get("type_sortie")
        new_type = classify_run_type(activity)

        # Appliquer systématiquement la nouvelle classification
        ...
        # IMPORTANT: Effacer session_category pour forcer recalcul avec nouveau type
    ...
    short_term_objectives = load_short_term_objectives()
    ...
    try:
        return render_template(
            ...
            short_term=short_term_objectives,
            activities_for_carousel=activities_for_carousel,
            current_weekly_program=weekly_program,
            profile=profile,
            shoe_km=shoe_km,
        ...
        app.logger.error(f"Erreur lors du rendu du template index.html: {e}", exc_info=True)
        # Retourne un message d'erreur plus clair à l'utilisateur
        return f"<h1>Erreur interne du serveur</h1><p>Un problème est survenu lors de l'affichage de la page. Détails: {e}</p>", 500
...
def force_week_transition():
    ...
    try:
        ...
        profile = load_profile()
        activities = load_activities_from_drive()
        activities = enrich_activities(activities, profile)
...
def generate_ai_comment(activity_date):
    ...
    """
    try:
        # Charger profil et enrichir les activités
        profile = load_profile()
        activities = load_activities_from_drive()
        activities = enrich_activities(activities, profile)  # 🆕 Enrichir TOUTES les activités avec cardiac_analysis
...
@app.route("/recompute_cardiac_zones")
def recompute_cardiac_zones():
    """Recalcule les zones FC avec formule Karvonen pour toutes les activités."""
    try:
        profile = load_profile()
        activities = load_activities_from_drive()
        print(f"♻️ Recalcul zones FC Karvonen pour {len(activities)} activités")
        print(f"   FC repos: {profile.get('hr_rest', 59)} bpm, FC max: {profile.get('hr_max', 170)} bpm")

        recalculated = 0
        for i, act in enumerate(activities):
            cardiac_analysis = analyze_cardiac_health(act, profile)
            if cardiac_analysis:
                act['cardiac_analysis'] = cardiac_analysis
                recalculated += 1

                # Afficher le premier pour debug
                if i == 0:
                    zone_pcts = cardiac_analysis.get('hr_zones', {}).get('zone_percentages', {})
                    print(f"   Exemple 1ère activité ({act.get('date')}):")
                    for z in range(1, 6):
                        print(f"     Z{z}: {zone_pcts.get(z, 0):.1f}%")

        print(f"💾 Sauvegarde de {len(activities)} activités dans activities.json...")
        save_activities_to_drive(activities)
        print(f"✅ activities.json mis à jour avec nouvelles zones FC ({recalculated} activités)")

        # Vérifier que ça a bien été sauvegardé
        activities_check = load_activities_from_drive()
        first_ca = activities_check[0].get('cardiac_analysis', {})
        zone_pcts_check = first_ca.get('hr_zones', {}).get('zone_percentages', {})
        print(f"   Vérification après reload - 1ère activité:")
        for z in range(1, 6):
            print(f"     Z{z}: {zone_pcts_check.get(z, 0):.1f}%")

        return f"✅ Zones FC recalculées pour {recalculated} activités avec Karvonen (hr_rest={profile.get('hr_rest', 59)}, hr_max={profile.get('hr_max', 170)})"
    except Exception as e:
        import traceback
        traceback.print_exc()
        print(f"❌ Erreur recalcul zones: {e}")
        return f"❌ Erreur: {e}", 500
...
@app.route('/program-guide')
def program_guide():
    """Page de documentation sur la fabrication des programmes"""
    return render_template('program_guide.html')
