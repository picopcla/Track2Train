...
def format_allure(allure_decimal):
    """Convertit une allure décimale en format mm:ss"""
    if allure_decimal is None:
        return "N/A"
    minutes = int(allure_decimal)
    seconds = int((allure_decimal - minutes) * 60)
    return f"{minutes}:{seconds:02d}"


def display_stats(stats_by_type):
    """Affiche les stats de manière lisible"""
    print("\n" + "=" * 100)
    print("📊 STATISTIQUES PAR TYPE DE RUN (15 dernières courses)")
    print("=" * 100)

    for run_type, stats in stats_by_type.items():
        print(f"\n🏃 {run_type.upper()} ({stats['nombre_courses']} courses)")
        print(f"   Dernière course: {stats['derniere_date']}")
        print(f"   Distance: {stats['distance']['moyenne']} km (range: {stats['distance']['min']}-{stats['distance']['max']})")

        if stats['fc_moyenne']['moyenne']:
            print(f"   FC moyenne: {stats['fc_moyenne']['moyenne']:.0f} bpm (range: {stats['fc_moyenne']['min']:.0f}-{stats['fc_moyenne']['max']:.0f})")

        if stats['fc_max']['moyenne']:
            print(f"   FC max: {stats['fc_max']['moyenne']:.0f} bpm (range: {stats['fc_max']['min']:.0f}-{stats['fc_max']['max']:.0f})")

        if stats.get('fc_segments'):
            segments_str = ', '.join([f"{fc:.0f}" if fc else "N/A" for fc in stats['fc_segments']])
            print(f"   FC segments: [{segments_str}] bpm")

        if stats['allure']['moyenne']:
            allure_moy = format_allure(stats['allure']['moyenne'])
            allure_min = format_allure(stats['allure']['min'])
            allure_max = format_allure(stats['allure']['max'])
            print(f"   Allure: {allure_moy}/km (range: {allure_min}-{allure_max}/km)")

        if stats['k_moy']['moyenne']:
            tendance = "📈" if stats['k_moy']['tendance'] == 'hausse' else "📉"
            print(f"   k_moy: {stats['k_moy']['moyenne']:.2f} (range: {stats['k_moy']['min']:.2f}-{stats['k_moy']['max']:.2f}) {tendance}")

        if stats['deriv_cardio']['moyenne']:
            deriv_status = "🟢" if stats['deriv_cardio']['moyenne'] < 1.15 else "🟡" if stats['deriv_cardio']['moyenne'] < 1.25 else "🔴"
            print(f"   Dérive cardio: {stats['deriv_cardio']['moyenne']:.3f} (range: {stats['deriv_cardio']['min']:.3f}-{stats['deriv_cardio']['max']:.3f}) {deriv_status}")
